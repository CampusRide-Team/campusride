...
const RootNavigator = () => {
  ...
  if (screen === "driver-login") {
    return (
      <DriverLogin
        onDriverLogin={(authPayload) => {
          // SECURITY GATEWAY VERIFICATION
          if (authPayload?.success) {
            if (setRole) setRole("driver");
            // Set local view driver profile dynamically from backend data upon entry
            if (authPayload.data?.user) {
              setDriverProfileData({
                fullName: authPayload.data.user.fullName,
                email: authPayload.data.user.email,
                avatarUri: null,
              });
            }
            setScreen("home");
  ...
  // 6. Driver Registration — Stage 1 (Personal Info)
  ...
  // 7. Driver Registration — Stage 2 (Vehicle Info)
  ...
  // 8. Driver Registration — Stage 3 (Document Submissions Gateway)
  ...
  if (screen === "driver-reg-success") {
    return (
      <DriverRegisterSuccess onBackToLogin={() => setScreen("driver-login")} />
    );
  }
    ...
    return (
      <DriverNavigation
        ...
        onArrive={() => {
          ...
          const durationCalculated = tripStartTime
            ? Math.round(totalElapsedMs / 60000)
            : 12;
          ...
          const dynamicTripPayload = {
            ...
            destinationLocation:
              activeTripData?.destination || "Student Union North",
            ...
            [
              {
                ...
                onPress: () => {
                  if (activeTripData) {
                    setRequestsPool((currentPool) =>
                      currentPool.filter(
                        (request) => request.id !== activeTripData.id,
                      ),
                    );
                  }
    ...
    return (
      <DriverProfile
        ...
        onNavigate={(targetTab) => {
          ...
          if (targetTab === "active-requests")
            return setScreen("active-requests");
